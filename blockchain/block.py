...
class Block(ABC, persistent.Persistent):
    ...
    def is_valid(self):
        ...
        if not self.is_genesis:
            ...
            # parent_block=chain.blocks[self.parent_hash]
            if not self.seal_is_valid() or self.target != parent_block.target:
        ...
        for tx in self.transactions:
            ...
            seen_tx_hashes.add(tx.hash)  # Add transaction to seen set

            # already exists'

            if tx.hash in chain.blocks_containing_tx:
                existing_blocks = chain.blocks_containing_tx[
                    tx.hash
                ]  # Get blocks containing this tx
                chain_hashes = chain.get_chain_ending_with(
                    self.parent_hash
                )  # Get all blocks in this chain

                # intersection
                if nonempty_intersection(existing_blocks, chain_hashes):
                    return False, "Double transaction inclusion"
            ...
            for input_ref in tx.input_refs:
                input_tx_id, output_index = input_ref.split(
                    ":"
                )  # Extract transaction ID and output index
                if input_ref in spent:
                    return False, "Double-spent input"

                spent.append(input_ref)

                input_tx = None
                for t in self.transactions:
                    if t.hash == input_tx_id:
                        input_tx = t
                        break

                if not input_tx:
                    if input_tx_id not in chain.all_transactions:
                    ...
                    # On failure: return False, "User inconsistencie"
                    input_tx = chain.all_transactions[input_tx_id]
                    found = False
                    for block_hash in chain.get_chain_ending_with(self.parent_hash):
                        if block_hash in chain.blocks_containing_tx[input_tx_id]:
                            found = True
                            break
                    if not found:
                        return False, "Input transaction not found"

                if int(output_index) >= len(input_tx.outputs):
                    return False, "Required output not found"

                referenced_output = input_tx.outputs[int(output_index)]
                if tx.outputs[0].sender != referenced_output.receiver:
                    return False, "User inconsistencies"
                for output in tx.outputs:
                    if tx.outputs[0].sender != output.sender:
                        return False, "User inconsistencies"

                input_users.add(referenced_output.receiver)
                ...
                if input_ref in chain.blocks_spending_input:
                    spent_blocks = chain.blocks_spending_input[input_ref]
                    chain_hashes = chain.get_chain_ending_with(self.parent_hash)

                    if nonempty_intersection(spent_blocks, chain_hashes):
                        return False, "Double-spent input"
                ...
                # if input_tx_id not in chain.blocks_containing_tx:
